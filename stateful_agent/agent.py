...
def save_state(data: dict) -> dict:
    ...
    STATE_FILE.parent.mkdir(parents=True, exist_ok=True) # Гарантує створення папки
    with open(STATE_FILE, 'w', encoding='utf-8') as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    return {"status": "saved", "data": data}

def remember_fact(key: str, value: str) -> dict:
    """Запам'ятовує факт про користувача"""
    state = load_state()
    state[key] = value
...
root_agent = Agent(
    ...
    name='stateful_agent',
    description="Агент який пам'ятає користувача між сесіями.",
    instruction="""
Ти персональний асистент який пам'ятає інформацію про користувача.
